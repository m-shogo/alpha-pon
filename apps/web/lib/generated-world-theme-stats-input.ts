...
export function isGeneratedWorldThemeStatsInput(value: unknown): value is WorldThemeCandidateStats {
  ...
  if (!rowsAreAtOrBeforeGeneratedAt(value.recent, value.generatedAt)) return false
  if (!value.byTheme.every((stat) => rowsAreAtOrBeforeGeneratedAt(stat.recent, value.generatedAt))) return false
