...
// retrospective chronological validationとprospective pre-outcome holdoutも分離する。
...
function render(date: string, payload: ReturnType<typeof buildPayload>): string {
  ...
  const lines = [
    ...
    "> retrospective chronological splitはtemporal robustness確認であり、prospective holdoutではありません。",
    "> outcome観測前登録のprospective holdoutを独立に満たすまでlocal thresholdはvalidatedへ昇格しません。",
    ...
    `- minimum retrospective train/temporal-validation: ${MIN_TRAIN_CASES}/${MIN_VALIDATION_CASES}`,
    `- minimum prospective holdout: ${MIN_PROSPECTIVE_HOLDOUT_CASES}`,
    ...
    `- research usable shadow 3m observations: ${payload.calibration.researchUsable3m}`,
    `- prospective holdout usable shadow 3m observations: ${payload.calibration.prospectiveUsable3m}`,
    `- below-threshold research shadow eligible/signals: ${lt12?.eligibleCases ?? 0}/${lt12?.cases ?? 0}`,
    `- >=12 research shadow eligible/signals: ${ge12?.eligibleCases ?? 0}/${ge12?.cases ?? 0}`,
    "",
    "## Threshold comparison snapshot — research scope only",
    "",
  ...
  lines.push("", "## 国別 readiness", "", "| country | market | level | status | research usable | country n | group n | temporal train | temporal validation | prospective holdout | threshold | source | registry |", "|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---|---|");
  for (const row of payload.countries) {
    const r = row.readiness;
    lines.push(`| ${r.country ?? "-"} | ${r.market ?? "-"} | ${r.modelLevel} | ${r.status} | ${r.usableOutcomeCases} | ${r.countryCases} | ${r.groupCases} | ${r.trainCases} | ${r.validationCases} | ${r.prospectiveHoldoutCases}/${r.prospectiveHoldoutRequired} | ${r.effectiveThreshold} | ${r.effectiveThresholdSource} | ${row.registryEntry?.id ?? "-"} |`);
  }

  lines.push("", "## 国×カテゴリ readiness（research usable shadow outcomeが存在する組合せのみ）", "");
  if (payload.countryCategories.length === 0) lines.push("- research usable shadow First Eligible Signal outcome未蓄積", "");
  for (const row of payload.countryCategories) {
    const r = row.readiness;
    lines.push(`### ${r.country ?? "-"} / ${r.category ?? "-"}`);
    lines.push(`- level/status: ${r.modelLevel} / ${r.status}`);
    lines.push(`- research n: country-category=${r.countryCategoryCases}, country=${r.countryCases}, group=${r.groupCases}`);
    lines.push(`- retrospective train/temporal-validation: ${r.trainCases}/${r.validationCases}`);
    lines.push(`- prospective holdout: ${r.prospectiveHoldoutCases}/${r.prospectiveHoldoutRequired} (${r.prospectiveHoldoutReady ? "ready" : "not ready"})`);
  ...
  lines.push("7. prospective holdoutはdefault calibration bucket / fitting sampleへ入れず、明示的なprospective評価scopeだけで評価する。");
  lines.push("8. retrospective researchではcountry-category → country → jurisdiction-group → globalの順でchronological temporal robustnessを確認する。");
  lines.push("9. temporal robustnessだけではvalidatedにしない。outcome観測前登録のprospective holdoutを最低8件通過する。");
  lines.push("10. prospective確認済み結果だけを `config/idiosyncratic-shock-calibration.yml` へ登録し、local threshold/weightを有効化する。");
  lines.push("11. registryが空・不一致ならglobal default 12へ縮退し、未来outcomeを過去scoreへ書き戻さない。");
  return lines.join("\n");
}
...
function buildPayload(date: string) {
  ...
  const usableObservations = observations.filter(row => Boolean(row.signalDate) && row.benchmarkRelative3m != null && Number.isFinite(row.benchmarkRelative3m));
  const researchUsableObservations = usableObservations.filter(row => row.validationHoldoutEligible !== true);
  const prospectiveUsableObservations = usableObservations.filter(row => row.validationHoldoutEligible === true);
  // calibrateShockThresholds defaults to research scope and therefore excludes prospective holdout.
  ...
  const countryCategories: ResolvedRow[] = [...new Set(researchUsableObservations
  ...
  return {
    ...
    calibration: {
      ...
      researchUsable3m: researchUsableObservations.length,
      prospectiveUsable3m: prospectiveUsableObservations.length,
    },
    thresholdBuckets,
    countries,
    countryCategories,
...
function main(): void {
  ...
  const ge12 = payload.thresholdBuckets.find(row => row.bucket === "score_ge_12");
  const lt12 = payload.thresholdBuckets.find(row => row.bucket === "score_lt_12");
  console.log(`shock calibration readiness: historical=${payload.historicalCaseCount} outcomes=${payload.outcomeRecordCount} production=${payload.production.pass}/${payload.production.block}/${payload.production.unknown} calibration=${payload.calibration.pass}/${payload.calibration.block}/${payload.calibration.unknown} shadowSignals=${payload.calibration.signals} researchUsable3m=${payload.calibration.researchUsable3m} prospectiveUsable3m=${payload.calibration.prospectiveUsable3m} >=12=${ge12?.eligibleCases ?? 0}/${ge12?.cases ?? 0} <12=${lt12?.eligibleCases ?? 0}/${lt12?.cases ?? 0} registry=${payload.validatedRegistryCount}`);
  for (const row of payload.countries.filter(row => row.readiness.country === "JP" || row.readiness.country === "US")) {
    const r = row.readiness;
    console.log(`  ${r.country}: ${r.modelLevel}/${r.status} researchN=${r.countryCases} prospective=${r.prospectiveHoldoutCases}/${r.prospectiveHoldoutRequired} threshold=${r.effectiveThreshold} source=${r.effectiveThresholdSource} registry=${row.registryEntry?.id ?? "-"}`);
  }
}
