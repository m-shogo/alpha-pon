...
function classifyQuality(company: Company, score?: LatestScore): BuffettQualitySnapshot {
  const text = [company.role, company.upsideHypothesis, company.noMoveHypothesis, company.downsideHypothesis, ...(company.evidenceToCheck ?? []), ...(score?.reasons ?? []), ...(score?.negativeReasons ?? []), ...(score?.warnings ?? [])].filter(Boolean).join(" ");
