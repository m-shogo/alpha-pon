...
function buildOutcomeNotes(input: { hypothesis: StockCandidateHypothesis; returns: ReturnData; relativeToTopix1m: number | null; result: HypothesisResult; dataSource: "jquants" | "mock" }): Pick<HypothesisOutcome, "whatMatched" | "whatDiffered" | "missedSignals" | "improvedRuleIdeas" | "notes"> {
  if (input.dataSource === "mock") return { whatMatched: [], whatDiffered: [], missedSignals: ["J-Quants未設定のため価格・TOPIX比を未検証"], improvedRuleIdeas: ["実データ取得後に同じ仮説を再レビューする"], notes: "J-Quants未設定のためリターン未計算" };
  ...
  if (input.hypothesis.expectedDirection === direction) whatMatched.push(`期待方向 ${input.hypothesis.expectedDirection} と1か月方向が一致`);
  else if (direction !== "unknown") whatDiffered.push(`期待方向 ${input.hypothesis.expectedDirection} に対して実績は ${direction}`);
  ...
  return { whatMatched, whatDiffered, missedSignals, improvedRuleIdeas: [...new Set(improvedRuleIdeas)], notes: `${todayJst()}時点で評価。1m=${input.returns.ret1m?.toFixed(1) ?? "N/A"}%, TOPIX比=${input.relativeToTopix1m?.toFixed(1) ?? "N/A"}%` };
}
