...
function assertStandaloneRegularFile(path: string): void {
  try {
    ...
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1) {
      throw new Error(`regime history must be a standalone regular file: ${path}`);
    }
  } catch (error) {
...
export function readRegimeHistoryLines(path: string): string[] {
...
export function replaceRegimeHistory(path: string, content: string): void {
  assertStandaloneRegularFile(path);
