...
import {
  validateDecisionFirewallLedgerGovernedStrict,
} from "./decision-firewall-governed.js";
import {
...
export type GovernedDecisionFirewallRepositoryOptions = {
  recordsPath?: string;
  ...
  includeReplayIssues?: boolean;
};

export type GovernedDecisionFirewallRepositoryResult = {
  issues: CouncilIssue[];
  recordCount: number;
...
export function validateGovernedDecisionFirewallRepository(
  options: GovernedDecisionFirewallRepositoryOptions = {},
): GovernedDecisionFirewallRepositoryResult {
  ...
  const issues: CouncilIssue[] = [
    ...(options.includeReplayIssues === false ? [] : replay.issues),
    ...recordRead.issues,
    ...manifestRead.issues,
  ];
  const schema = loadCouncilSchema(DECISION_FIREWALL_PATHS.schema);
  const manifests = new Map<string, CouncilReplayManifest>(
  ...
  issues.push(...validateDecisionFirewallLedgerGovernedStrict(
    recordRead.records,
    schema,
    manifests,
    replayResults,
