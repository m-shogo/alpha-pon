...
async function verifyFiles(
  ...
  for (const [index, value] of array(manifest.succeeded, "manifest.succeeded").entries()) {
    ...
    const binaryPath = directChild(
      directory,
      success.binaryFile,
      `manifest.succeeded[${index}].binaryFile`,
    );
    const metadataPath = directChild(
      directory,
      success.metadataFile,
      `manifest.succeeded[${index}].metadataFile`,
    );
...
function sourcePaths(manifest: JsonObject, directory: string): {
...
async function main(): Promise<void> {
  ...
  const paths = sourcePaths(manifest, directory);
