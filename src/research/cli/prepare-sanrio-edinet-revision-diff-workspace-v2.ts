...
} from "../edinet-sanrio-logical-entry-alignment.js";
...
async function comparePair(pair: SanrioEdinetRevisionPairPlan, acquisitionDirectory: string) {
  ...
  return compareSanrioEdinetRevisionEntriesWithLogicalAlignment({
    pair,
...
async function main(): Promise<void> {
  ...
  console.log("Sanrio EDINET correction diff extraction v2");
  ...
  console.log("alignment: exact path -> unique logical role -> unique identical content hash");
  ...
  for (let index = 0; index < plan.pairs.length; index++) {
    ...
    pairResults.push(result.diff);
    console.log(
      `modified=${result.diff.modifiedEntryCount} added=${result.diff.addedEntryCount} removed=${result.diff.removedEntryCount} `
      + `logicalAligned=${result.diagnostics.logicalRoleMatches} hashAligned=${result.diagnostics.identicalContentHashMatches}`,
    );
  }
  ...
  const jsonName = `revision-diff-workspace-v2.${stamp}.json`;
  const markdownName = `revision-diff-review-v2.${stamp}.md`;
  writeExclusiveDurable(resolve(acquisitionDirectory, jsonName), `${JSON.stringify(workspace, null, 2)}\n`);
  ...
  console.log(`changed PublicDoc entries after logical alignment: ${workspace.changedEntryCount}`);
...
main().catch(error => {
  ...
  console.error(`Sanrio EDINET revision diff v2 failed: ${message}`);
  process.exitCode = 1;
});
