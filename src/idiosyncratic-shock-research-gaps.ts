...
const COUNTRY_RAW_TARGET = 40; // 30 usable quantitative outcomesを作るための初期raw目標
const COUNTRY_USABLE_3M_TARGET = 30;
const COUNTRY_CATEGORY_RAW_TARGET = 25; // 20 usable outcomesを目指す余裕込み
...
function buildPayload(date: string) {
  ...
  const countryStats = PRIORITY_COUNTRIES.map(country => {
    ...
    const usable3m = quantitative.filter(row => row.benchmarkRelative3m != null && Number.isFinite(row.benchmarkRelative3m)).length;
  ...
  const priorities = [
    ...countryStats.flatMap(row => [
      row.usable3mGap > 0 ? { priority: 100 + row.usable3mGap, key: `${row.country}:quantitative`, reason: `usable 3m benchmark-relative outcomes ${row.usable3m}/${COUNTRY_USABLE_3M_TARGET}` } : null,
...
function render(payload: ReturnType<typeof buildPayload>): string {
  const lines = [
    ...
    "",
    "> ケース数を増やすこと自体が目的ではありません。国別キャリブレーションに必要なoutcome、失敗例、小反応/無反応例、context証拠を意図的に埋めます。",
    "",
  ...
  lines.push("", "## JP / US", "", "| country | raw | gap | quantitative | usable 3m | gap | <=5% shock controls | failed | high-confidence | context | reaction-anchor |", "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|");
  for (const row of payload.countryStats) {
    lines.push(`| ${row.country} | ${row.rawCases} | ${row.rawGap} | ${row.quantitativeCases} | ${row.usable3m} | ${row.usable3mGap} | ${row.smallOrNoShockRate}% | ${row.failedRate}% | ${row.highConfidenceRate}% | ${row.contextCoverage}% | ${row.reactionAnchorCoverage}% |`);
  }

  lines.push("", "## Culture-sensitive category gaps", "", "| country | category | sensitivity | raw | raw gap | usable 3m | failed |", "|---|---|---|---:|---:|---:|---:|");
  for (const row of payload.countryCategoryStats.filter(row => row.sensitivity === "high").sort((a, b) => b.rawGap - a.rawGap || a.country.localeCompare(b.country) || a.category.localeCompare(b.category))) {
    lines.push(`| ${row.country} | ${row.category} | ${row.sensitivity} | ${row.rawCases} | ${row.rawGap} | ${row.usable3m} | ${row.failed} |`);
  }
  ...
  lines.push("- raw case数よりusable quantitative outcome数を優先する。Local model昇格は3m benchmark-relative outcomeが必要。");
...
function main(): void {
  ...
  console.log(`shock research gaps: historical=${payload.totalHistoricalCases} quantitative=${payload.totalQuantitativeOutcomes} priorities=${payload.priorities.length} warnings=${payload.selectionBiasWarnings.length}`);
