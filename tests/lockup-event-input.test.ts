...
try {
  writeFileSync(path, [
    ...
    JSON.stringify({ id: "fractional-days", name: "Fractional", listingDate: "2026-07-01", lockupDays: 0.5 }),
    JSON.stringify({ id: "unsafe-days", name: "Unsafe", listingDate: "2026-07-01", lockupDays: Number.MAX_SAFE_INTEGER + 1 }),
    ...
    JSON.stringify({ id: "inconsistent-term", name: "Inconsistent", listingDate: "2026-07-01", lockupDays: 180, lockupExpiryDate: "2026-12-29" }),
    JSON.stringify({ id: "valid-2", name: "Valid 2", listingDate: "2026-07-01", lockupExpiryDate: "2026-12-28" }),
    JSON.stringify({ id: "valid-3", name: "Valid 3", listingDate: "2026-07-01", lockupDays: 180, lockupExpiryDate: "2026-12-28" }),
  ].join("\n"));

  const result = readListingEventRows<LockupMemo>(path, isLockupMemo);
  assert.deepEqual(result.rows.map(row => row.id), ["valid-1", "valid-2", "valid-3"]);
