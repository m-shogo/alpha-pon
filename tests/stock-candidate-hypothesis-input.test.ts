...
const specialWatchSource = readFileSync(new URL("../src/special-situation-watch-report.ts", import.meta.url), "utf-8");
assert.match(
  specialWatchSource,
  /parseHypothesisOutcomesJsonl\(readFileSync\(outcomePath, "utf-8"\), outcomePath\)/,
  "watch:specialもtested Outcome JSONL parserを再利用してmalformed rowを隔離する",
);
assert.match(
  specialWatchSource,
  /outcomeCoverageAudit\.notes\.push\(\.\.\.parsedOutcomes\.warnings\)/,
  "watch:specialはmalformed Outcomeをsilent dropせずcoverage auditのmetadata warningへ残す",
);
...
const universe = normalizeStockCandidateUniverseRows({
  ...
  candidates: [
    ...
    { ...validCandidate, code: "7203", detectedAt: "2026-02-31" },
    { ...validCandidate, code: "9984", detectedAt: "2999-01-01" },
  ],
});
assert.equal(universe.rootValid, true);
assert.deepEqual(universe.candidates.map(candidate => candidate.code), ["8136"], "malformed/non-canonical/PIT-invalid candidate rowや0〜100外scoreだけ隔離して正常candidateを保持する");
assert.equal(universe.warnings.length, 8, "malformed/non-canonical/PIT-invalid/out-of-range candidate rowをsilent dropしない");
