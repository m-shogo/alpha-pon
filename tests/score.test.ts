...
function testGeneratedWorldThemeCandidateHypothesisShape() {
  const valid = {
    sourceEventTitle: "event", sourceEventPublishedAt: "2026-08-17", theme: "theme", candidateCode: "8136", candidateCompany: "Sanrio",
    whyThisCompany: "why", upsideHypothesis: "up", downsideRisk: "down", nextPrimaryCheck: "check", reviewAfterDays: [30, 90, 180], disclaimer: "not advice",
  };
...
function testLearningScoreInputIsolation() {
  const valid = {
    code: "8136", name: "サンリオ", score: 72, alertLevel: "daily", createdAt: "2026-08-21", warnings: ["ok"], riskReview: { decision: "watch", blockers: [] },
...
function testLearningScorePitCutoff() {
  const parsed = parseLearningScoreInput(JSON.stringify([{ code: "8136", name: "サンリオ", score: 72, alertLevel: "daily", createdAt: "2026-08-22" }]), "2026-08-21");
  assert.ok(parsed);
...
function testLearningScoreDuplicateIdentity() {
  const base = { code: "8136", name: "サンリオ", score: 72, alertLevel: "daily", createdAt: "2026-08-21" };
  const parsed = parseLearningScoreInput(JSON.stringify([base, { ...base, score: 90 }, { ...base, code: "4661", name: "別銘柄" }]), "2026-08-21");
  assert.ok(parsed);
...
function testLearningAlertLevelContract() {
  const parsed = parseLearningScoreInput(JSON.stringify([{ code: "8136", name: "サンリオ", score: 72, alertLevel: "later", createdAt: "2026-08-21" }]), "2026-08-21");
  assert.ok(parsed);
...
function testHypothesisOutcomeReviewHorizonContract() {
  const base = { code: "8136", hypothesis: { detectedAt: "2026-08-21" }, reviewHorizon: "1w", actionLabel: "log", result: "unknown" };
