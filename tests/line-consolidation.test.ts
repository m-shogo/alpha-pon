...
// =========================================================================
// A. Builder: 決定論・重複排除・文字数予算・巨大fragment隔離
// =========================================================================
{
  assert.equal(buildConsolidatedMessage([], { today: TODAY }).message, null);

  const one = buildConsolidatedMessage(
    [bf("📊 銘柄スコア", "7203 トヨタ 62点")],
    { today: TODAY },
  );
  ...
  const variant1: BatchFragment = {
    hash: "ffff",
    section: "💎 特殊状況",
    body: "・XYZ 監視 https://a/1",
  };
  const variant2: BatchFragment = {
    hash: "0001",
    section: "💎 特殊状況",
    body: "・XYZ  監視  https://b/2",
  };
  assert.equal(
    normalizeKey(variant1.section, variant1.body),
    normalizeKey(variant2.section, variant2.body),
  );
  assert.equal(dedupeFragments([variant1, variant2]).representatives[0].hash, "0001");
  ...
  const allHuge = buildConsolidatedMessage(
    [
      bf("💎 特殊状況", `A${"P".repeat(20_000)}`),
      bf("💎 特殊状況", `B${"Q".repeat(20_000)}`),
    ],
    { today: TODAY, maxChars: 500 },
  );
...
// =========================================================================
// B. Transport / retry budget / secret redaction
// =========================================================================
{
  ...
  assert.ok(
    createTransport({
      ...
      LINE_USER_ID: "y",
      NOTIFY_MODE: "off",
    } as NodeJS.ProcessEnv) instanceof DryRunTransport,
  );
  assert.equal(
    createTransport({
      LINE_CHANNEL_TOKEN: "x",
  ...
  const throwingFetch = (async () => {
    throw new Error("down secretTOK");
  ...
  assert.ok(!redactSecrets("Bearer X to=Uabc", ["Uabc"]).includes("Uabc"));
...
withTempDir((dir) => {
  ...
  const result = enqueueFragment(dir, {
    text: "🌅 Alpha Pon Morning Lite\narray-ledger",
    kind: "normal",
    now: NOW,
  });
...
withTempDir((dir) => {
  const hNormal = putEnvelope(
    dir,
    "normal",
    "📊 銘柄スコア",
    "🌅 Alpha Pon Morning Lite\n1. 7203 62点",
  );
  const hScoreUrgent = putEnvelope(
    dir,
    "urgent",
    "🚨 緊急開示",
    "🚨 7203 トヨタ 88点\n  区分: 事実",
  );
  const hTdnet = putEnvelope(
    dir,
    "urgent",
    "🚨 緊急開示",
    "🚨 Alpha Pon 緊急開示\n・1234 TOB",
...
withTempDir((dir) => {
  ...
  writeFileSync(join(dir, "fragments", `${"b".repeat(16)}.fragment.json`), "{ broken");
  const ledger = reconcileOrphanFragments(dir, emptyLedger(), NOW);
  assert.deepEqual(Object.keys(ledger.entries), []);
  assert.equal(findLedgerAnomalies(dir, ledger).malformedEnvelopes.length, 1);
});
...
// =========================================================================
withTempDir((dir) => {
  const { hash, action } = enqueueFragment(dir, {
    text: "🌅 Alpha Pon Morning Lite\nT",
    kind: "normal",
    now: NOW,
  });
...
await withTempDir(async (dir) => {
  writeFileSync(join(dir, ".ledger-blocked.json"), JSON.stringify({
    reason: "ledger-corrupt",
    detectedAt: NOW,
  }));
  const enqueued = enqueueFragment(dir, {
    text: "🌅 x\nbody",
    kind: "normal",
    now: NOW,
  });
  ...
  const text = "🚨 Alpha Pon 緊急開示\n・1 TOB";
  const result = await deliverUrgent(dir, transport, {
    text,
...
await withTempDir(async (dir) => {
  ...
  const transport: LineTransport = {
    ...
    async send() {
      inspected = true;
      assert.equal(readEnvelope(dir, hash)?.kind, "urgent");
      assert.equal(readLedgerState(dir).ledger.entries[hash]?.status, "queued");
      return OK;
    },
  };
  const result = await deliverUrgent(dir, transport, {
...
await withTempDir(async (dir) => {
  ...
  await assert.rejects(deliverUrgent(dir, transport, {
    text,
    messages: [{ type: "text", text }],
  }));
  assert.equal(readEnvelope(dir, hash)?.kind, "urgent");
...
await withTempDir(async (dir) => {
  ...
  for (let i = 0; i < 3; i++) {
    assert.equal(
      (await deliverUrgent(dir, transport, {
        text: "🚨 U1",
        messages: [{ type: "text", text: "🚨 U1" }],
      })).outcome,
      "dry-run",
    );
  }
...
await withTempDir(async (dir) => {
  ...
  for (let i = 0; i < 6; i++) {
    assert.equal(
      (await deliverUrgent(dir, noCredentials, {
        text: "🚨 U2",
        messages: [{ type: "text", text: "🚨 U2" }],
      })).outcome,
      "credentials-missing",
    );
  }
...
await withTempDir(async (dir) => {
  ...
  const hash = contentHash(text);
  assert.equal(
    (await deliverUrgent(dir, fakeTransport("real", HTTP4), {
      text,
      messages: [{ type: "text", text }],
    })).outcome,
    "http-4xx",
  ...
  assert.equal(
    (await deliverUrgent(dir, fakeTransport("real", OK), {
      text,
      messages: [{ type: "text", text }],
    })).outcome,
    "sent",
  ...
  const text2 = "🚨 U4";
  assert.equal(
    (await deliverUrgent(dir, fakeTransport("real", NET), {
      text: text2,
...
// =========================================================================
// G. pipeline lock: ownership / stale / signal終了
// =========================================================================
{
  const lockScript = (body: string) => execSync(
    `bash -c 'set -u; source "${REPO}/scripts/pipeline-lock.sh"; ${body}'`,
    { encoding: "utf-8" },
  );

  {
    ...
    const out = lockScript(
      `Lk="${lock}"; pl_acquire "$Lk" && echo A1_OK; ` +
      `( pl_acquire "$Lk" && echo A2_GOT || echo A2_SKIP ); ` +
      `pl_release; pl_acquire "$Lk" && echo A3_OK; pl_release`,
    );
  ...
  // live PID + tokenは奪わない。
  {
    ...
    const out = lockScript(
      `Lk="${lock}"; mkdir "$Lk"; echo $$ > "$Lk/pid"; ` +
      `echo live-token > "$Lk/token"; pl_acquire "$Lk" && echo GOT || echo BLOCKED`,
    );
    assert.ok(out.includes("BLOCKED"));
    rmSync(base, { recursive: true, force: true });
  }

  // PID/token欠落の初期化途中lockは奪わない。
  {
    const base = mkdtempSync(join(tmpdir(), "locki-"));
    const lock = join(base, "lock.d");
    const out = lockScript(
      `Lk="${lock}"; mkdir "$Lk"; pl_acquire "$Lk" && echo GOT || echo BLOCKED; ` +
      `test -d "$Lk" && echo PRESERVED`,
    );
    assert.ok(out.includes("BLOCKED"));
  ...
  // 有効なtokenを持つ死亡PIDだけstale退避して再取得。
  {
    ...
    const lock = join(base, "lock.d");
    const out = lockScript(
      `Lk="${lock}"; mkdir "$Lk"; echo 999999 > "$Lk/pid"; ` +
      `echo stale-token > "$Lk/token"; echo ${NOW} > "$Lk/started_at"; ` +
      `pl_acquire "$Lk" && echo REACQUIRED || echo STILL_LOCKED; pl_release`,
    );
  ...
  // tokenが入れ替わっていたら他ownerのlockを削除しない。
  {
    ...
    const lock = join(base, "lock.d");
    const out = lockScript(
      `Lk="${lock}"; pl_acquire "$Lk"; echo replacement-token > "$Lk/token"; ` +
      `pl_release; test -d "$Lk" && echo PRESERVED`,
    );
    assert.ok(out.includes("PRESERVED"));
    rmSync(base, { recursive: true, force: true });
  }

  // TERM後はlockを解放してexit 143。改行でbackground構文を正しく区切る。
  {
...
// =========================================================================
{
  const emergency = readFileSync(
    new URL("../src/emergency-disclosure-watch.ts", import.meta.url),
    "utf-8",
  );
  assert.ok(
    emergency.includes("sendUrgentDisclosure") &&
    !emergency.includes("sendPipelineSummaryNotification"),
  );
  ...
  const complete = readFileSync(
    new URL("../scripts/run-daily-complete.sh", import.meta.url),
    "utf-8",
  );
  assert.ok(complete.includes("pl_acquire") && complete.includes("skipped_locked"));
  assert.ok(
    complete.includes("pl_exit_on_signal 130") &&
    complete.includes("pl_exit_on_signal 143"),
  );
}
