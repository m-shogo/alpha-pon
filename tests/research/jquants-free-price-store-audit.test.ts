...

{
  ...
  const report = auditJQuantsFreePriceStore({ root, schema });
  assert.equal(report.status, "issues_found");
  assert.equal(report.filesystemIssueCounts.unexpected_nested_entry, 1);
  assert.equal(report.fileCount, 0);
  assert.equal(report.recordCount, 0);
  assert.equal(JSON.stringify(report).includes("licensed-raw-line"), false);
  console.log("jquants-free-price-store-audit: nested directory is reported and never traversed OK");
}

{
  ...
  symlinkSync(outside, join(root, "latest.txt"));
  const report = auditJQuantsFreePriceStore({ root, schema });
  assert.equal(report.status, "issues_found");
  assert.equal(report.filesystemIssueCounts.unsafe_non_price_symlink, 1);
