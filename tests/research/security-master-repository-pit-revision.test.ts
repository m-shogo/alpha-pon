...
{
  const dir = mkdtempSync(join(tmpdir(), "security-master-repository-invalid-asof-"));
  try {
    const result = validateSecurityMasterRepository({
    ...
    assert.ok(result.issues.some((issue) => issue.code === "invalid_security_master_as_of"));
    assert.equal(result.snapshot.entities.length, 0);
    assert.equal(result.snapshot.relationships.length, 0);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
  console.log("security-master-repository: invalid asOf fails closed OK");
}
