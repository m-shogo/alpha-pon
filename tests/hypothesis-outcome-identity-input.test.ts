...
assert.deepEqual(jsonl.rows.map(row => row.code), ["8136"], "non-canonical code・不存在日・未来detectedAt・未知result・矛盾するhypothesis code・non-numeric maxDrawdownPctをOutcome履歴provenanceへ通さない");
assert.equal(jsonl.rows[0]?.result, "unknown", "legacy partial Outcomeのresult欠落はreview対象から消えないようcanonical unknownへ正規化する");
assert.match(jsonl.warnings[0], /8 malformed JSONL row\(s\).*line\(s\) 2, 3, 4, 5, 6, 7, 8, 9/);
...
assert.match(sqlite.warnings[0], /5 malformed record\(s\).*record\(s\) 2, 3, 4, 5, 6/);
