...
const record = buildShockHistoricalOutcome(strongCase, stock, topix, "2027-02-01");
...
assert.equal(record?.topixRelative1m, 6.6957);
assert.equal(record?.topixRelative3m, 15.5652);
...
assert((ge12?.avgTopixRelative1m ?? 0) > (lt12?.avgTopixRelative1m ?? 0));
