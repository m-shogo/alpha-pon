...
async function projection(env: Env): Promise<{
  schemaVersion: 1
  generatedAt: string
  source: 'cloudflare-d1'
  events: ProjectionEvent[]
  summary: Record<string, unknown>
  meta: { warnings: string[]; databasePath: null; calendarFeedConfigured: boolean }
}> {
  ...
  const priorityCounts = { S0: 0, S1: 0, S2: 0, S3: 0 }
  const decisionCounts = { BUY_WATCH: 0, WAIT: 0, BLOCK: 0, ABSTAIN: 0, INFO: 0 }
  ...
  const nextEventAt = events
    .filter(event => event.sortAt && !['COMPLETED', 'CANCELLED'].includes(event.status))
...
function calendarFeed(data: Awaited<ReturnType<typeof projection>>): string {
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
...
async function handle(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === '/healthz') {
    return json({ ok: true, accessConfigured: Boolean(env.OWNER_EMAIL), calendarFeedConfigured: Boolean(env.CALENDAR_FEED_TOKEN) })
  }
  ...
  if (url.pathname === '/api/market-events') return json(await projection(env))

  if (url.pathname === '/api/calendar-feed-url') {
  ...
  if (url.pathname.startsWith('/api/market-events/')) {
    const eventId = decodeURIComponent(url.pathname.slice('/api/market-events/'.length))
    const data = await projection(env)
...
export default {
  fetch(request: Request, env: Env): Promise<Response> {
    return handle(request, env).catch(error => {
      console.error('market-events-worker', error)
