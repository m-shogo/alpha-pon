...
#90 Sanrio legacy/configured parity human review
```
...
20. **COMPLETE — PR #90:** Finalize the parity workspace with explicit human legacy-to-configured mappings, configured-only dispositions, inventory-audit confirmation, and a human replacement recommendation. Even `recommend_configured_replacement` remains non-authorizing and does not mutate the legacy entry point.
21. **IN PROGRESS — PR #93:** Audit whether the completed configured/parity records actually supply the existing Foundation mapping contract. Verify nested hashes and lineage, expose missing Security Master/PIT/document-hash/parser-normalization/revision/license-storage/section evidence, never copy reviewed source text, and remain fail-closed.
22. Add the configured human-review-to-Foundation mapping gate only after a real local readiness audit proves all required mapping evidence exists. Do not fill audit gaps by inference or by substituting anchor hashes for document/section hashes.
23. Register a second real issuer only after an inventory-only proposal identifies a measured Evidence Gap and explicit user approval.
24. Resume Known-Bad Event Repricing validation only after the real Foundation pilot gates are satisfied.
...
- A hash-verified configured workspace, dashboard, synthetic fixture, fidelity plan, extraction bundle, anchor record, comparison report, human review record, parity workspace, parity human-review record, or Foundation readiness audit still requires the next governed gate before Foundation append or legacy replacement.
...
- Foundation readiness audit output must not copy reviewed source text or reviewed fact/prior-fact/assumption/opinion strings.
- Foundation readiness audit must not substitute line-level anchor hashes for document `sourceContentHash`, normalized structure hash, or semantic section content/title hashes.
- A readiness audit green execution proves only that the Evidence Gap calculation is reproducible. `foundationMappingGateReady` is not Foundation preview or append authorization.
