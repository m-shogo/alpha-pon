...
Base main: `16335c69b27157d2005e436f943e4f11ff4e74de`
...
It does **not** answer whether a proposal is correct, human-approved, persisted into the Catalog, merged as a duplicate, promoted to an Edge, or otherwise canonically resolved.
...
Two Markdown inputs are proposal-covered by Batch 007 as `component_candidate(kind=fixture)`. They remain input fixtures, not Study results or Edge evidence.
...
Both current Markdown analogs are proposal-covered by Batch 009 as bounded `case_candidate`s: eMnet Japan executive-control override and Tokyo Koki repeat-remediation.

### `docs/research/logs/`

Both current Markdown logs are proposal-covered by Batch 014: the JPX remediation-clock research log as existing-ResearchItem documentation candidate, and the expected-window calibration log as a calibration Component candidate. A research log is not automatically a Study or ResearchItem.

### `docs/research/fixtures/`

Current files are CSV/JSON, not Markdown. They are outside the current `scanDocumentPaths()` `.md` filter. If orphan discovery expands to CSV/JSON later, this audit must be regenerated rather than reused.
...
It does **not** mean human-approved, canonically classified, Asset-registered, Catalog-persisted, Relation-linked, duplicate-merged, Edge-promoted, or Research-Debt-resolved.
...
Until explicit human approval occurs, proposal coverage remains a read-only decision-support layer.
