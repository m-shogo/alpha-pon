...
Use this checklist when local Mac access becomes available again. Do not memorize or guess timestamped EDINET filenames. The canonical preflight inspects the actual local lineage and emits at most one safe next command.
...
4. Save/paste the complete preflight output into the Alpha Pon chat. It intentionally prints only stage, filenames/metadata, warnings, and the next command; it does not print filing source text, confirmed facts, or exact amounts.
...
6. Re-run the canonical preflight after every successful stage:
...
The known measured legacy state before local work resumes is:
...
STOP local progression and paste the full preflight output into the Alpha Pon chat.

Do not manually create a Foundation append, mutate the legacy entry point, rename/re-hash records, or synthesize missing Security Master/PIT/license/revision evidence. The next step must be assessed from the real completed parity evidence.

## Integrity-failure procedure

The canonical preflight now verifies deterministic content hashes and parent-hash lineage before it prints a next-stage command.
...
plus any integrity/finalization error printed by the command it instructed you to run. Do not paste raw EDINET filing text unless it is specifically needed for a human evidence decision.
