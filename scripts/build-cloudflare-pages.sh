...
echo "[1/9] Verify market event contracts"
...
echo "[2/9] Verify deterministic D1 bootstrap"
run_ts scripts/verify-d1-bootstrap-export.ts

echo "[3/9] Verify pre-Cloudflare readiness"
run_ts scripts/verify-cloudflare-calendar-readiness.ts

echo "[4/9] Build isolated market event snapshot"
...
echo "[5/9] Generate existing Alpha Pon web data"
pnpm ui:data

echo "[6/9] Typecheck web"
pnpm web:typecheck

echo "[7/9] Lint web"
pnpm --filter @alpha-pon/web lint

echo "[8/9] Build Next.js static export"
pnpm web:build

echo "[9/9] Verify Pages output"
