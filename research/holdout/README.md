...
# 同じ bundle を別の標本で確かめるなら --edge-id=<id>（事前登録が名乗っている id だけ）
```

- 引数だけなら**計画の表示**（価格は読まない）。`--execute` で1回だけ実行し、`access_log.jsonl` に追記する
- 開けない条件: 事前登録が未コミット・変更中・条件（bundle・開始日・営業日数・edgeId）を書いていない／
  この Edge が開封済み／確認期間の営業日が足りない／取り込みに穴がある／封印の窓と重ならない
- 確認期間の長さは**取り込まれた営業日の数**（`--trading-days`）か**終了日**（`--until`）で決める。
  「クラスタが N 以上」のように価格から数えると、開ける前に封印を覗くことになる。
  `--until` は取り込みがその日まで届いてから開く
...
- 事前登録は `- edgeId: \`<id>\`` と名乗ること。`--edge-id` はその id にしか使えない
  （同じ bundle を別の edgeId で開けるのを引数だけで決められると、「1 Edge 1回」を迂回できる）
